...
-(void)customizeiPadTheme
{
    ...
    [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0], //naslov boja
...
-(void)userLogIn
{
    // Kreiram login view kontroler
    logInViewController = [[PFLogInViewController alloc] init];
    [logInViewController setDelegate:self]; // Postavljam kao delegate da bi ga povezao sa aplikacijom

    
    // Ovdje kreiram sign up view kontroler
    signUpViewController = [[PFSignUpViewController alloc] init];
    [signUpViewController setDelegate:self]; // Postavljam kao delegate da bi ga povezao sa aplikacijom
    NSString *certificate = [self certificate];
    if(certificate != nil && [certificate length] > 0) {
        // dodatno polje za puno ime i prezime da bi kasnije bilo lakse unutar parse aplikacije da se vidi ko je sta polagao
        signUpViewController.fields = PFSignUpFieldsDefault | PFSignUpFieldsAdditional;
        signUpViewController.signUpView.additionalField.placeholder = NSLocalizedString(@"Puno ime i prezime", @"signup window");
    ...
    // Postavljam sing up kontroler da bude vidljiv i na login prozoru
    [logInViewController setSignUpController:signUpViewController];
    [logInViewController setFacebookPermissions:[NSArray arrayWithObjects:@"friends_about_me", nil]];
    if(allowAnonymous) {
        //postavljanje zeljenih prozora
    ...
    //postavke loga za sing up prozor
    ...
    // prezentuje login view kontroler
...
{
    // provjera korisnickog imena i passworda
    if (username && password && username.length != 0 && password.length != 0) {
        return YES; // pocinje logovanje
    }
    
    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Nedostaju informacije", @"login error title")
                                message:NSLocalizedString(@"Potrudite se da unesete sve potrebne informacije!", @"login error body")
                               delegate:nil
                      cancelButtonTitle:NSLocalizedString(@"Ok", @"login error button")
                      otherButtonTitles:nil] show];
    return NO; // prekini logovanje
}

// salje se u delegate kad je a PFUser ilogovan
...
// salje se u delegate kad se ne loguje
- (void)logInViewController:(PFLogInViewController *)logInController didFailToLogInWithError:(NSError *)error
{
    NSLog(@"Nemogucnost logovanja: %@", error);
}
...
// Salje se u delegate da bi se odredilo da li uopste trebaju informacije da se salju na server
- (BOOL)signUpViewController:(PFSignUpViewController *)signUpController shouldBeginSignUp:(NSDictionary *)info {
    BOOL informationComplete = YES;
    
    // vrtimo kroz sve podatke
    for (id key in info) {
        NSString *field = [info objectForKey:key];
        if (!field || field.length == 0) { // provjeravamo da li je zavrseno
    ...
    if (!informationComplete) {
        [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Nedostaju inormacije", @"login error title")
                                    message:NSLocalizedString(@"otrudite se da unesete sve potrebne informacije!", @"login error body")
                                   delegate:nil
                          cancelButtonTitle:NSLocalizedString(@"Ok", @"login error button")
                          otherButtonTitles:nil] show];
    }
...
// salje se u delegate kad se ne uspijemo logovat
- (void)signUpViewController:(PFSignUpViewController *)signUpController didFailToSignUpWithError:(NSError *)error
{
    NSLog(@"Nemogucnost logovanja: %@", error);
}

// salje se u delegate kad se prozor za logovanje prekine
...
-(void)startDownloadingCourse:(id)sender
{
    ...
    BlockAlertView *alert = [BlockAlertView alertWithTitle:NSLocalizedString(@"Početak preuzimanja", @"download confirmation") message:NSLocalizedString(@"Da li želite da preuzmete čitav sadržaj aplikacije?", @"download confirmation")];
    
    [alert setDestructiveButtonWithTitle:NSLocalizedString(@"Prekini", @"cancel downloading") block:nil];
    [alert addButtonWithTitle:NSLocalizedString(@"Preuzmi", @"") block:^{
...
-(void)downloadAllContent
{
    ...
    NSLog(@"going to download %lu remote files", (unsigned long)[remoteContent count]);
    for (NSString *url in [remoteContent allKeys]) {
        NSLog(@"> %@ used in %lu contents", url, (unsigned long)[[remoteContent valueForKey:url] count]);
    }
    if(nil == queue) {
...
-(void)allDownloadCompleted:(ASINetworkQueue*)queue
{
    if(!dlFailed && dlCompleted >= 0) {
        ...
        BlockAlertView *alert = [BlockAlertView alertWithTitle:NSLocalizedString(@"Preuzimanje završeno", @"download complete message") message:NSLocalizedString(@"Svi fajlovi su uspješno preuzeti", @"download complete message")];
        ...
        NSString *message = [NSString stringWithFormat:@"%d %@", files, NSLocalizedString(@"fajlovi nisu preuzeti", @"download error")];
        BlockAlertView *alert = [BlockAlertView alertWithTitle:NSLocalizedString(@"Greška", @"download error") message:message];
        [alert addButtonWithTitle:NSLocalizedString(@"OK", @"") block:^{
            NSLog(@"ok");
        }];
        [alert show];
