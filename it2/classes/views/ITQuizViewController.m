...
-(void)showNext
{
    ...
    if(currentQuestion >= [quizContent count]) {
        ...
        NSString *tt = [NSString stringWithFormat:NSLocalizedString(@"Vaš rezultat je: %i od mogućih %i ", @"quiz result"), score, [quizContent count]];
        [self.view addSubview:finalLabel];
        self.navigationItem.title = [NSString stringWithFormat:NSLocalizedString(@"Tačnih %i od %i", "quiz header"), score, [quizContent count]];
        if(_detailItem) {
        ...
        self.navigationItem.title = [NSString stringWithFormat:NSLocalizedString(@"Pitanje %i od %i", @"quiz header"), currentQuestion+1, [quizContent count]];
